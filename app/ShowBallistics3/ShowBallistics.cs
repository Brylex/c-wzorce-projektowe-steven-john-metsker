...
public class ShowBallistics : Form
{
    ...
    // funkcje
    private Tpeak _tPeak = new Tpeak();
    ...
    {
        Font = UI.NORMAL.Font;
        _burnRate = new BurnRate(_tPeak);
        _thrust = new Thrust(_tPeak);
    ...
    private Panel BurnRatePanel()
    {
        if (_burnRatePanel == null) 
        {
            _burnRatePanel = UI.NORMAL.CreatePaddedPanel(
                new AlertPlotPanel(NPOINT, _burnRate, _tPeak));
        }
    ...
    private Panel ThrustPanel()
    {
        if (_thrustPanel == null) 
        {
            _thrustPanel = UI.NORMAL.CreatePaddedPanel(
                new AlertPlotPanel(NPOINT, _thrust, _tPeak));
        }
        return _thrustPanel;
    }
    ...
    private TextBox ValueTextBox()
    {
        ...
        {
            _valueTextBox = new ValueTextBox(_tPeak);
    ...
    // Zadanie tej metody jest znacznie zawê¿one w porównaniu z pierwsz¹
    // wersj¹ aplikacji ShowBallistics. Zamiast aktualizowaæ wszystkie
    // kontrolki, metoda aktualizuje jedynie obiekt Tpeak.
